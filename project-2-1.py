...
if st.session_state.app_mode == "🏠 首頁 (漲跌排行榜)":
    ...
    st.info("💡 提示：點擊表格中的代號即可自動跳轉至個股深度分析。")
    ...
    # --- 篩選內容 (完全保留原本內容，僅改為水平排版或收納) ---
    with st.container(border=True):
        f_col1, f_col2, f_col3, f_col4 = st.columns([1, 1, 1, 1])
        with f_col1:
            st.session_state.market_type = st.radio("選擇市場", ["美股 (US)", "台股 (TW)"], horizontal=True, index=0 if st.session_state.market_type == "美股 (US)" else 1)
        with f_col2:
        ...
        with m_col3: run_analysis = st.button("🚀 開始分析", use_container_width=True)

    ticker_symbol, stock_name, full_data, dividends, fx_history = get_full_analysis(st.session_state.search_input, st.session_state.market_type, interval)
    ...
    if not full_data.empty:
        ...
        # (此處為您原本的 hover_dates 處理邏輯與圖表 Trace)
        hover_dates = []
        for d in plot_data.index:
            if interval == "1wk":
                s_w = d - timedelta(days=d.weekday()); e_w = s_w + timedelta(days=4)
                hover_dates.append(f"{s_w.strftime('%m/%d')} - {e_w.strftime('%m/%d')}")
        ...
        fig.add_trace(go.Candlestick(x=plot_data.index, open=plot_data['Open'], high=plot_data['High'], low=plot_data['Low'], close=plot_data['Close'], name="K線", customdata=hover_dates, hovertemplate="<b>時間: %{customdata}</b><br>收盤: %{close:.2f}<extra></extra>"), row=1, col=1)
        ...
        fig.add_trace(go.Bar(x=plot_data.index, y=plot_data['Volume'], name="成交量", marker_color="rgba(100,100,100,0.5)", hovertemplate="成交量: %{y:,.0f}<extra></extra>"), row=2, col=1)
        ...
        if not dividends.empty:
            ...
            for date, val in recent_divs.items():
                ...
                rows.append({"除息日期": date.strftime('%Y-%m-%d'), "推估發放日": est_pay_date, "配息": val, "預估年化殖利率": f"{(val * freq / curr_price) * 100:.2f}%"})
            st.table(pd.DataFrame(rows))
    else: st.error("查無數據，請確認代號正確性。")
