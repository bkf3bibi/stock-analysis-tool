...
if st.session_state.app_mode == "🏠 首頁 (漲跌排行榜)":
    ...
    if not full_data.empty:
        ...
        fig.add_trace(go.Candlestick(
            ...
            name="K線", customdata=hover_dates,
        ...
        fig.add_trace(go.Bar(
            x=plot_data.index, y=plot_data['Volume'], name="成交量", marker_color="rgba(100,100,100,0.5)",
        ...
        if not dividends.empty:
            ...
            for date, val in recent_divs.items():
                ...
                rows.append({"除息日期": date.strftime('%Y-%m-%d'), "推估發放日": (pd.to_datetime(date) + timedelta(days=offset)).strftime('%Y-%m-%d'), "配息": val, "預估年化殖利率": f"{(val * freq / curr_price) * 100:.2f}%"})
