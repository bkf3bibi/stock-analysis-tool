...
# 僅隱藏右上方工具欄與頁尾，保留側邊欄與標題功能
hide_st_style = """
            <style>
            /* 隱藏右上方工具欄 (GitHub, Edit, Settings) */
...
with st.sidebar:
    ...
    if st.session_state.app_mode == "📈 個股深度分析":
        st.header("🔍 投資參數設定")
        st.session_state.market_type = st.radio("選擇市場", ["美股 (US)", "台股 (TW)"],
                                                index=0 if st.session_state.market_type == "美股 (US)" else 1)
        ...
        ma_short_n = st.slider("短均線 (MA)", 5, 50, 20)
        ma_long_n = st.slider("長均線 (MA)", 20, 200, 60)
        run_analysis = st.button("🚀 開始分析", use_container_width=True)
    ...
    with st.spinner('正在分析市場動態...'):
        ...
        def show_clickable_table(df, title, is_us=False):
            ...
            if event.selection and len(event.selection.rows) > 0:
                ...
                selected_symbol = df.iloc[selected_idx]['代號']
                st.session_state.search_input = selected_symbol
    ...
    if not full_data.empty:
        ...
        if not dividends.empty:
            ...
            for date, val in recent_divs.items():
                ...
                est_pay_date = (target_date + timedelta(days=pay_offset)).strftime('%Y-%m-%d')
                freq = 12 if "00929" in ticker_symbol else (2 if any(x in ticker_symbol for x in ["0050", "0056"]) else 4)
                ann_yield = (val * freq / curr_price) * 100
                
                rows.append({
                    "除息日期": date.strftime('%Y-%m-%d'),
                    "推估發放日": est_pay_date,
                    "配息": val,
                    "預估年化殖利率": f"{ann_yield:.2f}%"
                })
